...
(function($) {
 var jsRoot = "static/component/filemanager/";
 var cssRoot = "static/component/filemanager/";
 var jspRoot = "static/component/filemanager/";
...
loadCSS = function(href) {
	...
	if($.inArray(href, HEAD_included_files) == -1) {
		var cssLink = $("<link rel='stylesheet' type='text/css' href='" + cssRoot + href + "'>");
		$("head").append(cssLink);
...
loadJS = function(src) {
	...
	if($.inArray(src, HEAD_included_files) == -1) {
		var jsLink = $("<script type='text/javascript' src='" + jsRoot + src + "'>");
	    $("head").append(jsLink);
...
var fileConnector = config.options.fileConnector || jspRoot + 'connectors/' + config.options.lang + '/filemanager.' + config.options.lang;
...
var lg = [];
var url = jsRoot+'scripts/languages/'  + config.options.culture + '.js';
$.ajax({
  url: url,
  async: false,
  dataType: 'json',
...
var getPdfReader = function(data) {
	var code  = '<iframe id="fm-pdf-viewer" src = "' + jsRoot + 'scripts/ViewerJS/index.html#' + data['Path'] + '" width="' + config.pdfs.pdfsReaderWidth + '" height="' + config.pdfs.pdfsReaderHeight + '" allowfullscreen webkitallowfullscreen></iframe>';
...
var editItem = function(data) {
	...
	isEdited = false;
		...
		$('#fileinfo').find('div#tools').append(' <a id="edit-file" href="#" onclick="return false;" title="' + lg.edit + '"><span>' + lg.edit + '</span></a>');
...
var getFileInfo = function(file) {
	...
	$.getJSON(fileConnector + '?mode=getinfo&path=' + encodeURIComponent(file)  + '&config=' + userconfig + '&time=' + d.getMilliseconds(), function(data){
		if(data['Code'] == 0){
			...
			if(data['Protected']==0) {
				$('#fileinfo').find('div#tools').append(' <a id="copy-button" data-clipboard-text="'+ url + '" title="' + lg.copy_to_clipboard + '" href="#"  onclick="return false;"><span>' + lg.copy_to_clipboard + '</span></a>');
				// loading zeroClipboard code
				
				loadJS('./scripts/zeroclipboard/copy.js?d' + d.getMilliseconds());
				$('#copy-button').click(function () {
					if(!$('#copied').is(":animated")) {
...
var getFolderInfo = function(path) {
	...
	var loading = '<img id="activity" src="' + cssRoot + 'themes/' + getTheme() + '/images/wait30trans.gif" width="30" height="30" />';
		...
		if(data){
			...
			if($('#fileinfo').data('view') == 'grid'){
				...
				for(key in data){
					...
					if(props['Size'] && props['Size'] != '') totalSize += parseFloat(props['Size']);
				...
				for(key in data){
					...
					if(props['Size'] && props['Size'] != ''){
						...
						totalSize += parseFloat(props['Size']);
					} else {
						result += '<td></td>';
					}
...
$(function(){
	...
	// Loading theme
	loadCSS('./themes/' + getTheme() + '/styles/filemanager.css');
	var cssUrl = cssRoot + './themes/' + getTheme() + '/styles/ie.css';
	$.ajax({
	    url:cssUrl,
	    async: false,
	    success: function(data)
